...
const getSearchBarcodeItemCodeDetails: any = async (
  ...
  await axios
    .get(
      `${CONSTANTS.API_BASE_URL}${CONSTANTS.API_METHOD_SDK}?version=v1&method=get_item_wise_barcode_filter&entity=barcode&posting_date=${request.date}&custom_karigar=${karigar_name}&name=${request.item_group}&sr_from=${request.sr_no_from}&sr_to=${request.sr_no_to}&stock=${request.stock}`,
      getHeaders
    )
    .then((res: any) => {
